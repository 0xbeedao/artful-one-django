...
class Base(Feed):
    ...
    def item_link(self, item):
        return (
            "https://artful.one" + item.get_absolute_url() + "#atom-%s" % self.ga_source
        )
...
class Entries(Base):
    title = "Bruce Kroeze's Weblog: Entries"
    ga_source = "entries"
    ...
    def item_description(self, item):
        note = (
            ...
            'Subscribe to <a href="https://artful.one/atom/everything/">/atom/everything/</a>'
            "to get all of my posts.</p>"
        )
        return item.body + note


class Blogmarks(Base):
    title = "Bruce Kroeze's Weblog: Blogmarks"
...
class SeriesFeed(Everything):
    ...
    def __init__(self, series):
        self.title = "Bruce Kroeze's Weblog: {}".format(series.title)
        self.series = series
...
class EverythingTagged(Everything):
    ...
    def __init__(self, title, items):
        self.title = "Bruce Kroeze's Weblog: {}".format(title)
        self._items = items
...
def sitemap(request):
    ...
    for klass in (Entry, Blogmark, Quotation, Note):
        for obj in klass.objects.exclude(is_draft=True).only("slug", "created"):
            xml.append(
                "<url><loc>https://artful.one%s</loc></url>" % obj.get_absolute_url()
            )
    xml.append("</urlset>")
