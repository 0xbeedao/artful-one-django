...
def wellknown_webfinger(request):
    remote_url = (
        "https://fedi.artful.one/.well-known/webfinger?" + request.META["QUERY_STRING"]
    )
...
def wellknown_hostmeta(request):
    remote_url = (
        "https://fedi.artful.one/.well-known/host-meta?" + request.META["QUERY_STRING"]
    )
    return proxy_view(request, remote_url)


def wellknown_nodeinfo(request):
    remote_url = "https://fedi.artful.one/.well-known/nodeinfo"
    return proxy_view(request, remote_url)


def username_redirect(request):
    return HttpResponseRedirect("https://fedi.artful.one/@bruce")


def projects_redirect(request):
    return HttpResponseRedirect("https://github.com/0xbeedao/")
...
Sitemap: https://artful.one/sitemap.xml
"""
